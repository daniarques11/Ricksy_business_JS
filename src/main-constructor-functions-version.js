...
var UfosPark = {
    ...
    flota: {},
    add: function(ufo) {
        if (!(ufo in this.flota)) {
            this.flota[ufo] = null;
        }
    },
    getUfoOf: function(cardNumber) {
        let ufoID = null;
        if (Object.values(this.flota).indexOf(cardNumber) > -1) {
            for (let ufo of Object.keys(this.flota)) {
                if (this.flota[ufo] == cardNumber) {
                    ufoID = ufo;
                    break;
                }
            }
...
function mostrarReserva(client) {
    ...
    console.log("Ufo: " + UfosPark.getUfoOf(client.number))
}
